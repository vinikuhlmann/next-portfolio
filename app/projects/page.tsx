  ...
  return (
    <div className="rounded-lg dark:bg-gray-800 p-5">
      ...
      <div className="px-3 py-4">
        ...
        <p className="text-gray-700 dark:text-white text-lg">{description}</p>
        <div className="py-2 flex flex-wrap gaṕ-2 justify-left">
          {tags &&
            tags.map((tag, index) => (
              <span
                key={index}
                className="bg-gray-200 rounded-full px-3 py-1 my-1 text-sm font-semibold text-gray-700 mr-2"
              >
                {tag}
              </span>
            ))}
